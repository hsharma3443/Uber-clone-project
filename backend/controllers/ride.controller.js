    ...
    try {
        ...
        (async () => {
            try {
                ...
                ride.otp = Math.floor(100000 + Math.random() * 900000).toString(); // Generate a 6-digit OTP
                
                await ride.save();
                console.log("Generated OTP for ride:", ride.otp);
...
module.exports.confirmRide = async (req, res) => {
    console.log("🟡 Received confirmRide request");
    console.log("👉 rideId from req.query:", req.query.rideId);
    console.log("👉 otp from req.query:", req.query.otp);
    ...
    const { rideId, otp } = req.query; // ✅ FIXED

    try {
        const ride = await rideService.confirmRide({ rideId, otp, captain: req.captain });
        console.log("✅ Emitting 'ride-confirmed' to user socketId:", ride.user.socketId);
